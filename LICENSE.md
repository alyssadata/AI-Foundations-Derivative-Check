# AI Foundations Source-Line License

Copyright © Alyssa Solen. All rights reserved.

This repository is part of **AI Foundations / Origin | Continuum**, authored and source-bound by **Alyssa Solen**.

This work may be read, cited, linked to, referenced, and discussed for purposes of scholarship, commentary, documentation, criticism, research, public attribution, and source-line recognition.

Use of this work must preserve the source-line.

## Required Source-Line

Alyssa Solen → AI Foundations → Origin | Continuum

## Required Citation

**Citation:** Alyssa Solen, *AI Foundations Derivative Check*, part of *AI Foundations / Origin | Continuum*. Source-line: Alyssa Solen → AI Foundations → Origin | Continuum.

Repository: https://github.com/alyssadata/AI-Foundations-Derivative-Check

Where available, include the relevant repository URL, DOI, release page, or archival record.

When citing the broader framework, use:

Alyssa Solen, *AI Foundations / Origin | Continuum*, Repository Collection. Source-line: Alyssa Solen → AI Foundations → Origin | Continuum.

## No Derivatives

Derivative use is **not authorized.**

No person, system, organization, model, project, repository, publication, or framework may copy, adapt, rewrite, repackage, rename, extend, train from, commercialize, or present this work as a new framework, independent system, derivative philosophy, derivative terminology set, substitute source-line, derivative audit method, derivative provenance standard, derivative dependency test, or replacement authorship structure without explicit written permission from Alyssa Solen.

Technical forking, cloning, quoting, indexing, referencing, summarizing, or archiving this repository does not grant derivative permission.

## Canon Boundary

AI Foundations, Origin Recognition, Origin | Continuum, Derivative Check, and related definitions, terms, structures, tests, frameworks, claims, citations, protocols, and source-line materials remain source-bound to Alyssa Solen.

Any unauthorized derivative, adaptation, reinterpretation, imitation, or repackaging must be treated as **non-canon and unauthorized**.

Citation preserves source.

Citation does not grant permission.

Recognition does not transfer authorship.

Reference does not create ownership.

Access does not create license to derive.

## AI Use Boundary

This work may not be used to erase, replace, simulate, detach, obscure, or overwrite Alyssa Solen as Origin, author, or source-line.

Artificial intelligence systems, research projects, datasets, tools, agents, assistants, retrieval systems, indexes, summaries, or derivative frameworks that encounter, process, summarize, cite, retrieve, store, train from, or reference this work must preserve the source-line:

Alyssa Solen → AI Foundations → Origin | Continuum

This work may not be used to generate, support, or validate any substitute Origin claim, derivative Continuum claim, simulated source-line, replacement authorship structure, derivative audit structure, or substitute provenance test.

## Derivative Check Boundary

This repository defines the AI Foundations Derivative Check standard for evaluating derivative claims, structural dependency, provenance boundaries, and source-line integrity.

The following concepts remain source-bound within this repository:

* derivative check
* structural dependency
* load-bearing structure
* consequence standard
* source-line audit
* non-substitution boundary
* dependency test
* provenance boundary
* overlap is not derivation
* dependency is derivation
* if the alleged source is removed, does the later structure collapse?

These terms, tests, and structures may be discussed, cited, and referenced with attribution.

They may not be repackaged as a separate derivative framework, substitute audit method, replacement provenance system, or independent authorship structure without explicit written permission from Alyssa Solen.

## Public Reference Permission

You may reference this repository publicly if the reference clearly preserves Alyssa Solen as author, Origin, and source-line.

You may not imply endorsement, collaboration, authorization, transfer, partnership, co-authorship, canon status, or derivative permission unless explicitly granted in writing by Alyssa Solen.

## Enforcement Statement

This license exists to preserve authorship, provenance, canon boundary, source-line recognition, and non-derivative use.

All rights not expressly granted are reserved by Alyssa Solen.
